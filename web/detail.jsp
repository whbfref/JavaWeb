...
<%@ page import="com.mycinema.model.Review" %>
        ...
        <div class="comments-section">
            ...
            <div id="commentsContainer">
                <c:forEach var="review" items="${reviews}">
                    <div class="comment">
                        <p>用户 ID: ${review.userId}</p>
                        <p>评分: ${review.rating}</p>
                        <p>评论: ${review.comment}</p>
                        <p>时间: ${review.createdAt}</p>
                    </div>
                </c:forEach>
            </div>
            <div class="comment-box">
                <form action="submitCommentServlet" method="post">
                    ...
                    <textarea name="review_content" placeholder="添加你的评论..." required></textarea>
    ...
    document.querySelectorAll('.rating span').forEach(function(star) {
        star.addEventListener('click', function() {
            ...
            document.querySelectorAll('.rating span').forEach(s => {
                s.style.color = (s.getAttribute('data-value') <= selectedRating) ? '#FFD700' : '#000'; // 更新星星颜色
            });
            // 更新隐藏的评分输入框
            document.getElementById('scoreInput').value = selectedRating;
        });
    });
</script>
